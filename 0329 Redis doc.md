...
패스워드를 지정하여 허용된 사용자만 DB에 접근하게 설정하고 포트을 변경한다

```jsx
# i 입력하여 편집모드로 변경 후 아래 부분 수정한다
$ sudo vim /etc/redis/redis.conf
```

- bind 0.0.0.0 ::1 ...(생략)
- requirepass [비밀번호 지정] ...(생략)
- port [포트지정]
- bindIp : 기본 127.0.0.1(localhost)로 설정되며 모든 IPv4를 허용하기 위해 0.0.0.0 으로 설정
- requirepass : redis db 접속 패스워드 지정
- port : 포트 설정
